...
export async function inserDocumentToCollection(client, collection, document) {
  const db = client.db();
  const resp = await db.collection(collection).insertOne(document);
  return resp;
}

export async function getAllDocumentsFromCollection(client, collection, filter={}) {
  const db = client.db();
  const collected = await db.collection(collection).find(filter).toArray();
  return collected;
}